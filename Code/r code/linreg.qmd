...
```{r}
transition_probs <- slot(fit@response, "transition")

# Extract transition coefficients
transition_coefs <- lapply(transition_probs, function(x) x$coefficients)

# Print the transition coefficients
for (i in seq_along(transition_coefs)) {
  cat("Transition from State", i, ":", paste(names(transition_coefs[[i]]), "*", round(transition_coefs[[i]], 4), collapse = " + "), "\n")
}
